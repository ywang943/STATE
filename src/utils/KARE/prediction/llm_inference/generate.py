...
# ────────────────────────────────────────────────────────────
# ────────────────────────────────────────────────────────────
def run_inference(samples: list, call_llm_fn, start_idx: int = 0, end_idx: int = None) -> list:
    if end_idx is None:
        end_idx = len(samples)
    ...
    for i, sample in enumerate(samples[start_idx:end_idx], start=start_idx):
        ...
        try:
            ...
            phase_a_messages  = build_reasoning_messages(sample)
            reasoning_chain   = call_llm_fn(
                phase_a_messages,
            ...
            phase_b_messages = build_prediction_messages(sample, reasoning_chain)
...
def main(start_idx: int = 0, end_idx: int = None):
    ...
    results = run_inference(samples, call_llm, start_idx=start_idx, end_idx=end_idx)
